...
export default function AddTranslationModal({ isOpen, onClose, getAllTranslations }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      axios
        .get(`${import.meta.env.VITE_REACT_APP_URL}/api/v1/admin/getAllAdmin`)
        .then((response) => {
          setHandlers(
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        onClose();
        getAllTranslations();
        // Redirect to AccountsPage with the selected client
        navigate("/dashboard/accounts", {
          state: {
            selectedClient: client,
            autoSelectClientId: client._id,
          },
        });
      }
    } catch (error: any) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <div className="relative">
                <Input {...register("nameInTargetScript")} className="pr-10 w-full" />
                <button
                  type="button"
                  onClick={handleCopyName}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors duration-200"
