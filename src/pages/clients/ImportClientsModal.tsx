...
export default function ImportClientsModal({
  ...
  const handleFileUpload = async (e) => {
    ...
    const toastId = toast.loading(" Please wait file Uploading..."); // Show loading toast
    ...
    try {
      Papa.parse(file, {
        complete: async (results) => {
          ...
          if (response.data.success) {
            getAllClients();
            toast.success("CSV data imported successfully", { id: toastId }); // Update toast to success
          }
        },
        header: false,
      });
    } catch (err) {
      toast.error("Error importing CSV data", { id: toastId }); // Update toast to error
    } finally {
      setLoading(false);
    }
  };
