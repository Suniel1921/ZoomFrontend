...
const ChatWithTeam = ({ onClose }) => {
  ...
  const connectWebSocket = useCallback(() => {
    ...
    websocket.onmessage = (event) => {
      ...
      if (data.type === 'CLIENT_MESSAGE' || data.type === 'CLIENT_REPLY') {
        setMessages(prev => {
  ...
  const handleSendMessage = () => {
    ...
    ws.send(JSON.stringify({ type: 'CLIENT_MESSAGE', content: message }));
    setMessage('');
  };
            ...
            <div className="flex flex-col">
              ...
              {msg.from._id === auth.user.id && (
                <div className="flex items-center justify-end mt-1">
                  ...
                  {msg.from.profilePhoto ? (
                    <img src={msg.from.profilePhoto} alt={msg.from.name} className="w-6 h-6 rounded-full" />
                  ) : (
